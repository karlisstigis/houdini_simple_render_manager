...
class QueueFilterProxyModel(QtCore.QSortFilterProxyModel):
    ...
    def _refresh_filter(self) -> None:
        ...
        if callable(begin) and callable(end) and direction is not None:
            begin()
            try:
                end(direction.Rows)
            except TypeError:
    ...
    def filterAcceptsRow(self, source_row: int, source_parent: QtCore.QModelIndex) -> bool:
        ...
        if self._status_filter:
            status_index = model.index(source_row, 6, source_parent)
            runtime_status = str(model.data(status_index, DISPLAY_STATUS_ROLE) or "")
            display_status = str(model.data(status_index, QtCore.Qt.ItemDataRole.DisplayRole) or "")
            if self._status_filter == "Disabled":
                if display_status != "Disabled":
                    return False
            elif runtime_status != self._status_filter:
                return False
